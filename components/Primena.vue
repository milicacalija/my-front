...
(url);
    ...
    response.data.data.forEach(item => {
      ...
      primene.forEach(prim => {
        ...
        // Dodajemo console.log da vidimo šta ide u mapu
        console.log("Povezujemo proizvod sa primenom:", {
          primena: prim,
    ...
    console.log("Konačne stavke (items) za prikaz:", this.items);
  ...
  if (this.imageMap[key]) {
    console.log("Image found for:", tpr_naziv);
...
.klik-iupac:hover {
 ...
 @media (max-width: 600px) {
  ...
  /* Popup lista proizvoda centrirana po ekranu */
  .proizvodi-lista {
    position: fixed; 
    top: 50%; 
    left: 50%; 
    ...
    display: flex;
    flex-direction: column;
    align-items: center;        /* centriranje proizvoda horizontalno */
    justify-content: flex-start;
    border-radius: 10px;
    box-shadow: 0 5px 20px rgba(0,0,0,0.3);
  }

  /* Pojedinačni proizvod u listi */
  .proizvod-item {
    width: 100%;
    max-width: 220px;
    text-align: center;
    margin: 0 auto 8px auto; /* margin auto centriranje */
  }
}
