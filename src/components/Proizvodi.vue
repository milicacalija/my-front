...
const nar_datum = moment().tz('Europe/Belgrade').format('YYYY-MM-DD HH:mm:ss');
  ...
  logger.log('Šaljem narudžbinu sa načinom plaćanja:', nacinPlacanja);
    ...
    const response = await api.post('/narudzbenice', {
...
async fetchCartItems() {
  try {
    const response = await api.get('/narudzbenice');
...
toggleCartPopup() {
  ...
  if (isMobile) {
    // Mobilni: Swal.fire popup
    const cartItems = JSON.parse(localStorage.getItem('cart')) || [];
    const itemsMap = JSON.parse(localStorage.getItem('itemsMap')) || {};

    let htmlContent = '';

    if (cartItems.length === 0) {
      htmlContent = '<p>Korpa je prazna</p>';
    } else {
      htmlContent = '<ul style="list-style: none; padding: 0;">';

      cartItems.forEach(item => {
        const product = itemsMap[item.fk_stv_pro_id] || {};
        const naziv = product?.pro_iupac || item?.pro_iupac;

        if (!naziv) {
          console.warn('⚠️ Stavka nema pro_iupac za:', item);
        }
      ...
      const totalPrice = cartItems.reduce(
        (total, item) => total + ((item.stv_cena || 0) * (item.stv_kolicina || 0)),
        0
      );
      htmlContent += `<p><strong>Ukupna cena: ${totalPrice.toFixed(2)} RSD</strong></p>`;

     // Unutar toggleCartPopup() za mobilni
Swal.fire({
  title: 'Vaša korpa',
  html: htmlContent + `
  ...
  showConfirmButton: false, // uklonimo default confirm dugme
  width: '90%',
  didOpen: () => {
    // Priključimo JS event listener na dugme unutar Swal
    const btn = document.getElementById('swal-go-to-checkout');
    if (btn) {
      btn.addEventListener('click', () => {
        this.goToCheckout();  // poziv Vue metode
        Swal.close();          // zatvori Swal
      });
    }
  }
});
...
.cart-popup {
  max-width: 350px;     /* širina korpe */
  max-height: 80vh;     /* maksimalna visina da ne pređe ekran */
  overflow-y: auto;     /* dodaj scroll ako sadržaj prelazi visinu */
  padding: 15px;
  border-radius: 8px;
...
@media (max-width: 768px) {
  ...
  /* Sakrij side-drawer korpu na mobilnom */
.cart-popup {
  max-width: 350px;     /* širina korpe */
  max-height: 80vh;     /* maksimalna visina da ne pređe ekran */
  overflow-y: auto;     /* dodaj scroll ako sadržaj prelazi visinu */
  ...
  /* Primer: redosled elemenata unutar popup-a */
  .cart-popup .cart-item {
    display: flex;
    flex-direction: column;
    margin-bottom: 15px;
  }
/* Kada se aktivira preko klik logike */
.cart-popup.show {
